...
import pickle
import os
def right_result(year,month,show_num):
    file_path='./static/img/r'
    ...
    for i in range(len(df3_pred)):
        ...
        df3_pred.loc[i,['예측점수']] = round(tmp_score,1)*(-100) #13은 예측점수 위치
    ...
    test=df3_pred.loc[:,['병원기호','요양기관명','이상여부','예측점수','총청구건수']]
    
    print(test)
    content_list={}
    content_list['result']=test.values.tolist()[show_num]
    
        
    
    print('sdifojasdfkaslnkdfl;kasd',content_list)
    ...
    for i in range(3):
        ...
        for t in range(round(content_top_outlier[i]*100)):
            try:
                #print(t,'\n',(score_dic[name]*100)+((content_top_outlier[i]*100-score_dic[name]*100)/round(content_top_outlier[i]*100))*t)
        ...
        plt.text(content_top_outlier[i]*100,1,round(content_top_outlier[i]*100,1),fontsize=10)
        
        plt.savefig('static/img/r/'+str(show_num)+'_'+str(i)+'.png')
        plt.show()
        print('save!!!!!!!!!!!!!')
        
    content_list['name0']=content_top_outlier_index[0]
    content_list['name1']=content_top_outlier_index[1]
    content_list['name2']=content_top_outlier_index[2]
    content_list['num0']=str(show_num)+'_'+str(0)
    content_list['num1']=str(show_num)+'_'+str(1)
    content_list['num2']=str(show_num)+'_'+str(2)
