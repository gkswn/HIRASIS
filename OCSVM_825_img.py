...
# 파일 불러오기 (학습 데이터)
#===========================================================
path = "C:\\Users\\jochangheon\\Desktop\\web\\input\\2022\\7월" #경로설정
df_origin = pd.read_csv(path+'\\df.txt', sep=',',encoding='cp949') #학습데이터이터
...
try :
    df = df_origin.drop(['병원기호','종별코드','이상여부'],axis=1) #구분기호 제거
except : #이상여부가 없어서 날아갈때
...
# print(df2.head())
# print(df2_pred)
#df2_pred된것을 병원기호 기준으로 df2원본에 업데이트하여 새 기준 마련
df2 = pd.concat([df2,df2_pred])
path2 = "C:\\Users\\jochangheon\\Desktop\\web\\학습이후 결과" #경로설정
df2.to_csv(path2+"\\결과.txt",index=False)
# 위에 기존거 합친 결과를 다시학습시키면 그게 의료환경 변화하는것으로 판단 가능

#-----------df2 저장해두고 다음에 불러오면 갱신된 프로그램 확인 가능--------------
...
#===========================================================
# print(df3)
# print(df3_pred) #병원이 정규화된것과 이상정도

#정답수치↓---------------------------------------------------------------------
df_nomal = df3[df3['이상여부']==1].reset_index() #이상없는것만 일단 가져옴

std = list(df_nomal.drop(['index','병원기호','이상여부'],axis=1).std(axis=0))
...
score_dic['평균내원일수'] = 0
score_dic['청구총금액75%'] = 0
score_dic['기본진료소계금액평균'] = 0
score_dic['평균청구원내외래약제비금액'] = 0
score_dic['응급진료자비율'] = 0
score_dic['상병수'] = 0
score_dic['진료행위수'] = 0
score_dic['진료당진료금액'] = 0
...
for i in range(len(df_nomal)):
    tmp_line = df_nomal.iloc[i] #한줄
    #----------입원환자 비율-------------------------
    score_dic['입원환자비율']+=tmp_line['입원환자비율']
    #----------평균내원일수-------------------------
    score_dic['평균내원일수']+=tmp_line['평균내원일수']
    #----------청구총금액75%-------------------------        
    score_dic['청구총금액75%']+=tmp_line['청구총금액75%']
    #----------기본진료소계금액평균-------------------------
    score_dic['기본진료소계금액평균']+=tmp_line['기본진료소계금액평균']
    #----------평균청구원내외래약제비금액-------------------------
    score_dic['평균청구원내외래약제비금액']+=tmp_line['평균청구원내외래약제비금액']
    #----------응급진료자비율-------------------------
    score_dic['응급진료자비율']+=tmp_line['응급진료자비율']
    #----------상병수-------------------------
    score_dic['상병수']+=tmp_line['상병수']
    #----------진료행위수-------------------------
    score_dic['진료행위수']+=tmp_line['진료행위수']
    #----------진료당진료금액-------------------------
    score_dic['진료당진료금액']+=tmp_line['진료당진료금액']
    #----------행위2부위평균-------------------------
    score_dic['행위2부위평균']+=tmp_line['행위2부위평균']

score_dic['입원환자비율']/=len(df_nomal)
score_dic['평균내원일수']/=len(df_nomal)
score_dic['청구총금액75%']/=len(df_nomal)
score_dic['기본진료소계금액평균']/=len(df_nomal)
score_dic['평균청구원내외래약제비금액']/=len(df_nomal)
score_dic['응급진료자비율']/=len(df_nomal)
score_dic['상병수']/=len(df_nomal)
score_dic['진료행위수']/=len(df_nomal)
score_dic['진료당진료금액']/=len(df_nomal)
score_dic['행위2부위평균']/=len(df_nomal)

#정답수치↑---------------------------------------------------------------------


#===========================================================
# 낮은쪽 이상여부 제거하기(이상이 아니기 때문!)
#===========================================================
for i in range(len(df3_pred)):
    c = 0
    for j in df3_pred.columns[1:-1]:
        if (list(score_dic.values())[c]) > (df3_pred.iloc[i][j]) :
            df3_pred.loc[i,[j]] = list(score_dic.values())[c]
            df3_pred.iloc[i]['이상여부']=1 #고쳐야함. 하나라도 낮은거 있으면 그 부분 전부가 정상치로 바뀌어버리는 잘못 짠 코드
        c+=1
...
#--------------병원명 추가---------------------------------------------------
try : #encoding 에러 확인
    input_hospitalname = pd.read_csv(path+'\\가_병원명.txt', sep=',',encoding='cp949')
except :
    input_hospitalname = pd.read_csv(path+'\\가_병원명.txt', sep=',',encoding='UTF8')

df3_pred= pd.merge(df3_pred,input_hospitalname,how='left',on='병원기호')
...
#예측점수 연산
for i in range(len(df3_pred)):
    tmp_line = df3_pred.iloc[i] #한줄
    tmp_score = 0
    #----------입원환자 비율-------------------------
    tmp_score+=(score_dic['입원환자비율']-tmp_line['입원환자비율']) * (1-df_std.iloc[0][0])
    #----------평균내원일수-------------------------
    tmp_score+=(score_dic['평균내원일수']-tmp_line['평균내원일수']) * (1-df_std.iloc[1][0])
    #----------청구총금액75%-------------------------        
    tmp_score+=(score_dic['청구총금액75%']-tmp_line['청구총금액75%']) * (1-df_std.iloc[2][0])
    #----------기본진료소계금액평균-------------------------
    tmp_score+=(score_dic['기본진료소계금액평균']-tmp_line['기본진료소계금액평균']) * (1-df_std.iloc[3][0])
    #----------평균청구원내외래약제비금액-------------------------
    tmp_score+=(score_dic['평균청구원내외래약제비금액']-tmp_line['평균청구원내외래약제비금액']) * (1-df_std.iloc[4][0])
    #----------응급진료자비율-------------------------
    tmp_score+=(score_dic['응급진료자비율']-tmp_line['응급진료자비율']) * (1-df_std.iloc[5][0])
    #----------상병수-------------------------
    tmp_score+=(score_dic['상병수']-tmp_line['상병수']) * (1-df_std.iloc[6][0])
    #----------진료행위수-------------------------
    tmp_score+=(score_dic['진료행위수']-tmp_line['진료행위수']) * (1-df_std.iloc[7][0])
    #----------진료당진료금액-------------------------
    tmp_score+=(score_dic['진료당진료금액']-tmp_line['진료당진료금액']) * (1-df_std.iloc[8][0])
    #----------행위2부위평균-------------------------
    tmp_score+=(score_dic['행위2부위평균']-tmp_line['행위2부위평균']) * (1-df_std.iloc[9][0])
    df3_pred.loc[i,['예측점수']] = round(tmp_score,3)*(-100) #13은 예측점수 위치
...
df3_pred['총청구건수']=0
count_200 = {}
for i in range(len(input_200T)):
    tmp_line = input_200T.iloc[i] #한줄
    h_code=tmp_line['YKIHO']
    if h_code not in count_200:
        count_200[h_code]=0
    count_200[h_code]+=1

for i in range(len(df3_pred)):
    df3_pred.loc[i,['총청구건수']] = count_200[df3_pred.iloc[i]['병원기호']]
print(df3_pred.columns)
...
# final_df=df3_pred[['입원환자비율', '평균내원일수', '청구총금액75%', '기본진료소계금액평균', '평균청구원내외래약제비금액', '응급진료자비율', '상병수', '진료행위수', '진료당진료금액', '행위2부위평균']]
...
# print('1',final_df.columns)
# print('\n')
# print('2',final_df.iloc[show_num])
# print('\n')
# print('3',final_df.iloc[show_num].to_list())
# print('\n')

# pred_list=final_df.iloc[show_num].to_list()


# test_df=[]
# name_list=final_df.columns.to_list()

# for i in range(len(pred_list)):
#     test_df.append(pred_list[i]-score_dic[name_list[i]])
    
# print('test_dftest_dftest_dftest_df',test_df)    
    
# count_idx=[]
# ori_pred_list=test_df[:]
# num_tmp=test_df[:]
# top_num=[]
# top_idx=[]
...
# #이상치가 하나일 떄와 두개일 때
# for e in range(3):
#     top_num.append(test_df.pop())
#     top_idx.append(num_tmp.index(top_num[e]))
# print(final_df.columns)
# #해당행에서 상위3개 , 상위 3개의 index
# print('num',top_num,'\n','idx',top_idx)
# print(test_df)

# content_top_outlier=[]
# content_top_outlier_index=[]
# for i in top_idx:
#     content_top_outlier.append(ori_pred_list[i])
#     content_top_outlier_index.append(final_df.columns.to_list()[i])
# print("해당행 이상치값 ",content_top_outlier)
# print('해당행 컬럼 이름',content_top_outlier_index)
...
#     fig,ax=plt.subplots()
#     ax.plot()
#     ax.add_patch(patches.Rectangle((0,0),100,3,fill=False))
#     print('실행중인 컬럼이름',content_top_outlier_index[i])
#     name=content_top_outlier_index[i]
#     ax.vlines(score_dic[name]*100,0,3)
    
    
#     x1 = []
#     for g in range(round((content_top_outlier[i]*100))):
#         x1.append(g+1)
#     cm = plt.get_cmap("Reds")
#     cm_interval = [g/(len(x1)) for g in range(1, len(x1)+1)]
#     cm = cm(cm_interval)
    
#     for t in range(round(content_top_outlier[i]*100)):
#         try:
#             print('1111111',score_dic[name])
#             print('222222222',content_top_outlier[i])
#             shptmp = patches.Rectangle(((score_dic[name]*100)+((content_top_outlier[i]*100-score_dic[name]*100)/round(content_top_outlier[i]*100))*t,0),(content_top_outlier[i]*100-score_dic[name]*100)/round(content_top_outlier[i]*100),3,edgecolor='none', facecolor = cm[t])
#             plt.gca().add_patch(shptmp)
#         except:
#             pass  
#     plt.title(content_top_outlier_index[i],fontdict=font)
#     plt.savefig('./static/img/'+str(show_num)+'_'+str(i)+'.png')
#     plt.axis('off')
#     plt.show()